...
public function obtenerFallasFiltradas($filtros) {
    $sql = "SELECT f.*, p.nombre as prioridad, e.nombre as estado 
            FROM fallas f
            LEFT JOIN prioridades p ON f.id_prioridad = p.id
            LEFT JOIN estados e ON f.id_estado = e.id
            WHERE 1=1";
    ...
    if (!empty($filtros['fechaInicio']) && !empty($filtros['fechaFin'])) {
        $sql .= " AND f.fecha_hora_reporte BETWEEN ? AND ?";
        $params[] = $filtros['fechaInicio'];
        $params[] = $filtros['fechaFin'];
    }
    
    // Otros filtros
    $columnasValidas = ['nombre_equipo', 'nombre_subsistema', 'id_falla' /*...*/];
    foreach($filtros as $columna => $valor) {
        if(in_array($columna, $columnasValidas) && $valor) {
            $sql .= " AND $columna = ?";
            $params[] = $valor;
        }
    }
